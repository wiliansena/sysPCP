document.addEventListener("DOMContentLoaded", function () {
    ...
    // Função para adicionar itens na tabela de edição
    ...
    // Evento para atualizar valores ao modificar consumo ou produção
    ...
    // Evento para remover itens sem apagá-los imediatamente
    document.addEventListener("click", function (event) {
        if (event.target.classList.contains("remover-item")) {
            let row = event.target.closest("tr");
            row.style.display = "none";  // Oculta visualmente
            row.querySelectorAll("input").forEach(input => input.disabled = true);
            atualizarResumo();
        }
    });

    // Eventos para os botões de seleção dos modais
    document.addEventListener("click", function (event) {
    ...
    // Certifica que os inputs escondidos sejam enviados corretamente
